...
export default function DeleteSessionButton({ sessionId }: { sessionId: string }) {
  ...
  return (
    <button
      ...
      className="rounded-lg p-1.5 text-slate-600 hover:bg-red-500/10 hover:text-red-400 transition-colors flex-shrink-0"
      title="Deletar sessão de estudo (remove também as revisões geradas)"
    >
      <Trash2 size={15} />
    </button>
  )
